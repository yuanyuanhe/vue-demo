<template>
 ...
 <h1>产品详情</h1>
 <router-link to="cart" class="topCart"><em>{{buycart}}</em></router-link>
</header>
...
import toCart from "./detailtocart"
export default {
  data(){
    return {
      obj:this.obj,
    ...
    var params = new URLSearchParams();
      this.$http({
        ...
        if(!cookie.getCookie("pid")){
          var arr = []
          arr.push(this.pid)
          cookie.setCookie("pid",arr,700);
        }else{
          var str = cookie.getCookie("pid")
          // console.log(str.split(","))
          var arr2 = str.split(",")
          arr2.push(this.pid);
          cookie.setCookie("pid",arr2,700);
        }
