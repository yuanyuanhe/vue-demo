  ...
  <div class="goods">
      <ul>
        <li v-for="val in arr1" :key="val.pid">
          <router-link :to="{name:'product',params:{uid:'1'},query:{pid:val.pid}}" class="goodsPic">
          <img :src="val.src"/>
          </router-link>
          <div class="goodsInfor">
          <h2>
            <router-link :to="{name:'product',params:{uid:'1'},query:{pid:val.pid}}">{{val.name}}</router-link>
          </h2>
          <p>
  ...
  mounted(){
    var params = new URLSearchParams();
      this.$http({
          ...
          url:'http://localhost:8003/hotgoods',
          method:'post',
          data:params
      }).then((res)=>{
        console.log(res)
